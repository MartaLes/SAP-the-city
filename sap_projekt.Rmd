...
depresije prikladno raditi linearnu regresiju. Prema tome, nezavisne
...
Za potrebe analize zasebnog utjecaja svake od ovih varijabli,
procijeniti ćemo jednostavne **modele linearne regresije**, pri čemu će
se za svaku nezavisnu varijablu izgraditi poseban model.
...
Iduće, pretpostavljene zaključke dobivene analizom grafova linearne
regresije potvrditi ćemo izračunom **Pearsonovih korelacijskih
koeficijenata**.
...
```{r}
#Zakomentiramo ispis ovih testova da bi izbjegle redundantnost ispisa
...
pozitivne povezanosti između makijavelizma, narcizma i psihoticizma (r u
rasponu 0,36–0,42, p \< 0,001). Iako su ove osobine povezane, snaga
korelacija ne sugerira problematičnu multikolinearnost, što podupire
uključivanje njih kao zasebnih prediktora u regresijski model.

Sada možemo definirati model **višestruke linearne regresije**, gdje
gledamo sveukupni utjecaj mračne trijade na depresiju.
...
makijavelizam ($\beta$ = 0.21, p \< 0.001) i psihoticizam ($\beta$ =
0.28, p \< 0.001) pozitivno značajni prediktori. Model je statistički
značajan (F-statistic = 25.92, p \< 0.001). Međutim, ukupna prediktivna
snaga modela je umjerena (R-squared oko 0.16), što sugerira da postoje i
drugi faktori koji utječu na depresiju.
...
Model značajno predviđa depresiju (R-squared= 0.42, p \< 0.001).
Neuroticizam je bio najsnažniji pozitivni prediktor depresije ($\beta$ =
0.46, p \< 0.001), dok je ekstraverzija imala značajan negativan učinak
($\beta$ = -0.24, p \< 0.001). Psihoticizam je također pokazao umjeren,
ali značajan pozitivan doprinos ($\beta$ = 0.17, p = 0.001). Narcizam,
savjesnost i ugodnost nisu bili statistički značajni prediktori (p \>
0.05), što upućuje na to da njihov odnos s depresijom nije jedinstven
nakon kontrole ostalih osobina.
...
Još je preostalo provjeriti **pretpostavke regresija**, tj. prate li
reziduali ovih modela normalnu distribuciju.
...
# #gplot(df_sex, aes(sample = agreeableness)) +
#   stat_qq() + stat_qq_line() +
#   facet_wrap(~sex)
# 
...
Distribucije su provjerene za sve osobine među spolovima i za sve
osobine vrijedu da priližno slijede normalnu distribuciju. Zatim ćemo
provjeriti varijance.
...
neuroticizmu (t = 2.52, p = 0.012) i ugodnosti (t = 3.76, p \< 0.001).
...
istim dvjema nezavisnim skupinama ispitanik (Female i Male).
...
Distribucije u skupinama su priližno normalne. Test je proveden za sve
mračne osobine.
...
odnosu na žene, što upućuje na izraženije spolne razlike u ovoj
dimenziji ličnosti.
...
zaposlenih na puno radno vrijeme, što dodatno ukazuje na veliku
...
Za početak je provedena provjera pretpostavki normalnosti distribucije i
homogenosti varijanci za varijable anksioznost, depresiju i stres. U
nastavku su prikazani rezultati testova samo za skupine zaposlenih na
puno radno vrijeme i studenata, budući da te skupine imaju najveći broj
ispitanika te omogućuju pouzdaniju procjenu distribucije podataka.
...
# Lilliefors test po skupinama zanimanja (samo Full-Time i Student)
...
Iz dobivenih ispisa zaključujemo da na razini značajnosti $\alpha$ =
0.05 nema potrebe za odbacivanjem nulte hipoteze o normalnosti
distribucije anksioznosti na razini cijelog uzorka (p \> 0.05). Na
temelju grafičke analize histograma može se zaključiti da distribucije
anksioznosti za skupine zaposlenih na puno radno vrijeme i studenata
približno slijede normalnu distribuciju. Uočena manja odstupanja od
savršene simetrije nisu izražena te se ne smatraju statistički
relevantnima.

Iako su testovi normalnosti i grafičke provjere provedeni za sve
skupine, u radu su prikazani samo rezultati za skupine zaposlenih na
puno radno vrijeme i studenata radi veće preglednosti. Postoje manja
odstupanja od simetrije u preostale dvije skupine, ali to je opravdano
manjim brojem članova tih skupina.
...
Lillieforsov test normalnosti ukazuje na statistički značajno odstupanje
distribucije depresije od normalnosti na razini cijelog uzorka, kao i
unutar skupina zaposlenih na puno radno vrijeme i studenata (p \< 0.05).
Kod studentske skupine odstupanje je granično, što je u skladu s
vizualnom inspekcijom histograma koji upućuju na približno simetričnu,
ali blago nesimetričnu distribuciju.

Unatoč utvrđenim odstupanjima od normalnosti, primjena parametarskih
metoda smatra se opravdanom s obzirom na veličinu uzoraka te poznatu
robusnost linearne regresije i ANOVA-e na umjerena kršenja pretpostavke
normalnosti.
...
Test normalnosti proveden je za varijablu stresa na razini cijelog
uzorka, kao i unutar skupina prema radnom statusu. Rezultati
Lillieforsovog testa pokazuju da nijedan od provedenih testova nije
statistički značajan (p \> 0.05), kako na ukupnoj razini, tako i unutar
promatranih skupina.

Vizualni pregled histograma za skupine zaposlenih na puno radno vrijeme
i studenata dodatno potvrđuje ove nalaze, budući da distribucije ne
pokazuju izražena odstupanja od normalnosti. Manja odstupanja od
simetrije mogu se uočiti u skupinama s manjim brojem ispitanika
(zaposleni na nepuno radno vrijeme i nezaposleni), no takva odstupanja
vjerojatno su posljedica veće varijabilnosti uzrokovane manjom veličinom
uzorka, a ne stvarnih odstupanja od normalne distribucije.
...
definiranih prema radnom statusu (occupation_group). Prije provođenja
ANOVA-e ispitane su pretpostavke normalnosti distribucije i homogenosti
varijanci za sve promatrane varijable. Iako su kod pojedinih varijabli
uočena manja odstupanja od normalnosti, s obzirom na veličinu uzorka i
robusnost ANOVA-e na umjerena kršenja ove pretpostavke, primjena
parametarskih metoda smatrana je opravdanom.

Rezultati jednosmjerne ANOVA-e nisu pokazali statistički značajne
razlike u razini anksioznosti između skupina radnog statusa, F(3, 366) =
1.258, p = 0.288. Također, nisu utvrđene statistički značajne razlike u
razini stresa, F(3, 366) = 0.4944, p = 0.687.

Za razinu depresije uočen je trend prema razlikama između skupina, no
rezultat nije dosegnuo konvencionalnu razinu statističke značajnosti,
F(3, 366) = 2.29, p = 0.078. Dobiveni nalazi upućuju na to da radni
status sam po sebi nije snažan prediktor razine mentalnog zdravlja u
promatranom uzorku.
...
U svrhu ispitivanja odnosa između radnog statusa i anksioznosti proveden
je model višestruke linearne regresije kojim se ispituje doprinos radnog
statusa povrh crta ličnosti.

Rezultati pokazuju da je regresijski model statistički značajan u
cjelini, F(12, 357) = 7.79, p = 6.73x$10^{-13}$. Model objašnjava oko
21% varijance anksioznosti (R-squared= 0.2074), dok prilagođeni
koeficijent determinacije iznosi 0.1808. Iako je objašnjeni udio
varijance manji u usporedbi s modelima za stres i depresiju, dobivene
vrijednosti upućuju na to da model ima smisla za interpretaciju.

Analizom mračnih crta ličnosti nije utvrđen nijedan statistički značajan
jedinstveni prediktor anksioznosti. Narcizam, makijavelizam,
psihoticizam i sadizam nisu pokazali značajan doprinos nakon kontrole
ostalih crta ličnosti, što sugerira da se njihova povezanost s
anksioznošću ne zadržava u ovom multivarijatnom modelu.

Unutar petofaktorskog modela ličnosti, neuroticizam se jasno istaknuo
kao ključni prediktor anksioznosti ($\beta$ = 0.37, p \< 0.001). Osobe s
višim razinama neuroticizma pokazuju izraženiju anksioznost, neovisno o
radnom statusu i ostalim osobinama ličnosti, što je u skladu s
postojećim teorijskim i empirijskim nalazima. Ekstraverzija nije bila
statistički značajan prediktor anksioznosti (p = 0.118), kao ni ostale
Big Five crte ličnosti.
...
Kako bi se ispitao doprinos radnog statusa razini depresije uz kontrolu
crta ličnosti, proveden je model višestruke linearne regresije.

Regresijski model pokazao se statistički značajnim u cjelini, F(12, 357)
= 26.33, p \< 2.2x$10^{-16}$. Model objašnjava približno 47% varijance
depresije (R-squared= 0.4695), dok prilagođeni koeficijent determinacije
iznosi 0.4517, što upućuje na vrlo dobru objašnjavajuću snagu modela.

Među mračnim crtama ličnosti, psihoticizam se pokazao statistički
značajnim pozitivnim prediktorom depresije ($\beta$ = 0.134, p = 0.029),
što sugerira da osobe s izraženijim obilježjima psihoticizma imaju višu
razinu depresije i nakon kontrole ostalih osobina ličnosti i radnog
statusa. Makijavelizam je pokazao slab, ali statistički neznačajan trend
($\beta$ = 0.088, p = 0.093), dok narcizam i sadizam nisu imali značajan
doprinos u objašnjenju depresije.

Unutar petofaktorskog modela ličnosti, neuroticizam se ponovno istaknuo
kao najsnažniji pozitivni prediktor depresije ($\beta$ = 0.437, p \<
0.001), potvrđujući njegovu ključnu ulogu u objašnjenju emocionalnih
poteškoća. Za razliku od modela za anksioznost i stres, ekstraverzija se
ovdje pokazala statistički značajnim negativnim prediktorom depresije
($\beta$ = -0.306, p \< 0.001), što upućuje na niže razine depresije kod
osoba s višom razinom ekstraverzije. Ostale crte Big Five modela nisu se
